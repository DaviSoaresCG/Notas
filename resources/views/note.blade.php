<section class="w-full h-64 px-5 py-4 bg-gray-900 rounded-2xl mb-4">
    <div class="flex flex-row items-center justify-between pb-6 border-b-1 border-gray-300/25">
        <div class="flex flex-col gap-2 justify-center items-start ">
            ...
            <p>Criado em <strong>{{ date('d/m/Y', strtotime($note->created_at)) }}</strong> às
                <strong>{{ date('H:i:s', strtotime($note->created_at)) }}</strong>
            </p>
            @if ($note->created_at != $note->updated_at)
                <p>Modificado em <strong>{{ date('d/m/Y', strtotime($note->updated_at)) }}</strong> às
                    <strong>{{ date('H:i:s', strtotime($note->updated_at)) }}</strong>
                </p>
            @endif
        </div>
        <div class="flex flex-row gap-4">
            <a href=" {{ route('edit', ['id' => Crypt::encrypt($note->id)]) }}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="size-6 hover:text-blue-400">
